...
def parse_meals(document):
	meal_times = document.find_all(class_="splGroupWrapper")
	for time in meal_times:
		current_time = time.find(class_="splGroup")
		if not current_time:
			current_time = time.find(class_="splGroupAbendmensa")
		current_time = current_time.string.strip()
		meals = time.find_all(class_='rowMealInner')
		for meal in meals:
			...
			info = {
					'additives': additives_element.text.strip().replace("Zusatzstoffe: ", "").split(",") if additives_element and additives_element.text.strip() else [],
			...
			prices = price_regex.findall(prices_element.text) if prices_element else None

			# check for non-empty name
			if len(name) and name not in ["Desserttheke", "Salattheke"]:
	...
	def parse_single_date(self, date):
		...
		if not parse_closed(document):
			...
			for (name, info, prices) in parse_meals(document):
				...
				misc = ', '.join((item for item in info['misc'] if item not in ['Vegane Speisen (V*)', 'Vegetarische Speisen (V)', 'Fisch (F)']))

				notes = [info["time"], additives]
				...
				self.feed.addMeal(date.date(), category, name, notes=notes, prices=prices, roles=('student', 'employee', 'other'))
			return True
		else:
			return False
